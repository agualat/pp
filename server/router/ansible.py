...
router = APIRouter(
    prefix="/ansible", tags=["ansible"], dependencies=[Depends(get_current_staff_user)]
)
...
def patch_playbook(task_id: int, updates: dict, db: Session = Depends(get_db)):
    ...
    if not updated:
        raise HTTPException(
            status_code=status.HTTP_400_BAD_REQUEST,
            detail="Name already exists or invalid update",
        )
    return updated
...
def put_playbook_task_name(task_id: int, name: str, db: Session = Depends(get_db)):
    ...
    if not updated:
        raise HTTPException(
            status_code=status.HTTP_400_BAD_REQUEST,
            detail="Playbook not found or name already exists",
        )
    return updated
...
def remove_playbook(task_id: int, db: Session = Depends(get_db)):
    """Soft delete: marca un playbook como eliminado sin borrarlo físicamente"""
    deleted = delete_task(db, task_id)
    if not deleted:
        raise HTTPException(
            status_code=status.HTTP_404_NOT_FOUND, detail="Playbook not found"
        )
    return {"deleted": True, "message": "Playbook marked as deleted (soft delete)"}


@router.delete("/playbooks/by-name")
def remove_playbook_by_name(name: str, db: Session = Depends(get_db)):
    """Soft delete: marca un playbook como eliminado por nombre"""
...
@router.get("/playbooks/deleted", response_model=List[AnsibleTaskResponse])
def list_deleted_playbooks(
    skip: int = 0, limit: int = 100, db: Session = Depends(get_db)
):
    """Lista todos los playbooks eliminados (soft delete)"""
    return get_deleted_tasks(db, skip=skip, limit=limit)


@router.post("/playbooks/{task_id}/restore", response_model=AnsibleTaskResponse)
def restore_deleted_playbook(task_id: int, db: Session = Depends(get_db)):
    """Restaura un playbook eliminado (soft delete)"""
    restored = restore_task(db, task_id)
    if not restored:
        raise HTTPException(
            status_code=status.HTTP_404_NOT_FOUND,
            detail="Playbook not found or already active",
        )
    task = get_task_by_id(db, task_id)
    return task
...
def bulk_create_playbooks(
    payload: List[AnsibleTaskCreate], db: Session = Depends(get_db)
):
...
def run_playbook(
    ...
    for sid in payload.server_ids:
        if not get_server_by_id(db, sid):
            raise HTTPException(
                status_code=status.HTTP_400_BAD_REQUEST,
                detail=f"Invalid server id {sid}",
            )
