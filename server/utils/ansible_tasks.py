...
def run_ansible_playbook(self, execution_id: int, dry_run: bool = False):
    ...
    try:
        ...
        # Construir hosts con configuración individual
        for server in servers:
            ...
            # Agregar become_password (usa la contraseña SSH guardada)
            if server.ssh_password_encrypted:
                ssh_pwd = decrypt_password(server.ssh_password_encrypted)
                if ssh_pwd:
                    host_config["ansible_become_password"] = ssh_pwd

            inventory["all"]["hosts"][server.name] = host_config
